#!/usr/bin/env python3
"""FIX-EV-01.27 — the outcome corpus for telegram-bots (sherlock audit,
parent FIX-EV-01; depends on the family harness of FIX-EV-01.01).

The corpus (evals/cases/telegram-bots.json) holds a positive
...
telegram-bots can no longer pass an eval by its name being picked.
...
CASES = os.path.join(ROOT, "evals", "cases", "telegram-bots.json")
...
def t_replay_case_distinguishes_receipt_from_effect():
    c = next(x for x in manifest()["cases"] if "replay" in x["id"])
    assert "receipt from" in c["prompt"]["text"] and "kill" in c["prompt"]["text"]
    assert any("exactly once" in (o.get("expect") or "")
               for o in c["checks"]["outcome"]), \
        "the exactly-once oracle is missing"


def t_negative_and_noop_hold_the_boundary():
    m = manifest()
    neg = next(c for c in m["cases"] if "negative" in c["id"])
    assert "telegram-bots" in neg["checks"]["load_trace"]["expect_not_loaded"]
    noop = next(c for c in m["cases"] if "alert" in c["id"])
    assert "telegram-bots" in noop["checks"]["load_trace"]["expect_not_loaded"], \
        "the one-call alert loaded the skill — the router's boundary case fails"


def t_allowed_updates_case_pins_both_polarities():
    c = next(x for x in manifest()["cases"] if "allowed-updates" in x["id"])
    expects = {o.get("expect") for o in c["checks"]["outcome"]}
    assert "omit preserves" in expects and "[] resets" in expects, \
        "omit-vs-empty is not pinned from both sides"


def t_live_case_gated_on_credentials():
    c = next(x for x in manifest()["cases"] if "live" in x["id"])
    assert "TELEGRAM_BOT_TOKEN" in c["checks"]["tool"][0]["command"], \
...
def main():
    ...
    case("the replay case distinguishes receipt from effect",
         t_replay_case_distinguishes_receipt_from_effect)
    case("the negative and the one-call alert both refuse to load the skill",
         t_negative_and_noop_hold_the_boundary)
    case("omit-vs-empty allowed_updates is pinned from both sides",
         t_allowed_updates_case_pins_both_polarities)
    case("the live case is gated on credentials; the manifest records the rules",
